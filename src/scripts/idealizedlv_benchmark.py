...
process_num = 6
directory_path = '../tests/idealized_LV/test_parameters/'
extension = 'prm'
parameters_files = list_files_with_extension(directory_path, extension)
build_dir = "../tests/idealized_LV/build/"
mesh_file_name = '../../../../lifex_fiber_generation_examples/mesh/idealized_LV.msh'
executable = "main"
...
if (os.system(f'cd {build_dir} && cmake .. && make -j {process_num}') != 0):
    ...
    for i, f in enumerate(parameters_files):
        f = f.replace('tests/idealized_LV/', '')
        print(f)
