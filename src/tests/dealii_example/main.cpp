...
// This is taken from:
// https://github.dev/dealii/code-gallery/blob/master/Quasi_static_Finite_strain_Compressible_Elasticity/cook_membrane.cc
...
void AssemblyMethod::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("Assembly method");
  {
    prm.declare_entry("Automatic differentiation order", "0",
                      ...
                      "The automatic differentiation order to be used in the "
                      "assembly of the linear system.\n"
                      "# Order = 0: Both the residual and linearisation are "
                      "computed manually.\n"
                      "# Order = 1: The residual is computed manually but the "
                      "linearisation is performed using AD.\n"
                      "# Order = 2: Both the residual and linearisation are "
                      "computed using AD.");
  }
...
void AssemblyMethod::parse_parameters(ParameterHandler &prm) {
  prm.enter_subsection("Assembly method");
  {
    automatic_differentiation_order =
        prm.get_integer("Automatic differentiation order");
  }
  prm.leave_subsection();
}
...
void Materials::declare_parameters(ParameterHandler &prm) {
  ...
  {
    prm.declare_entry("Poisson's ratio", "0.3", Patterns::Double(-1.0, 0.5),
                      "Poisson's ratio");

    prm.declare_entry("Shear modulus", "0.4225e6", Patterns::Double(),
                      "Shear modulus");
  }
  prm.leave_subsection();
}
...
void LinearSolver::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("Linear solver");
  {
    ...
    prm.declare_entry(
        "Max iteration multiplier", "1", Patterns::Double(0.0),
...
void NonlinearSolver::declare_parameters(ParameterHandler &prm) {
  prm.enter_subsection("Nonlinear solver");
  {
    ...
    prm.declare_entry("Tolerance displacement", "1.0e-6", Patterns::Double(0.0),
...
public:
  ...
  NumberType get_Psi(const NumberType &det_F,
...
private:
  ...
  void print_vertical_tip_displacement();
};
...
void Solid<dim, NumberType>::system_setup() {
  ...
  tangent_matrix.clear();
  {
    ...
    DoFTools::make_sparsity_pattern(dof_handler_ref, coupling, csp, constraints,
                                    false);
...
template <int dim, typename NumberType>
void Solid<dim, NumberType>::solve_nonlinear_timestep(
...
void Solid<dim, NumberType>::print_vertical_tip_displacement() {
  ...
  const Point<dim> soln_pt =
      (dim == 3 ? Point<dim>(48.0 * parameters.scale, 52.0 * parameters.scale,
...
template <int dim, typename NumberType>
void Solid<dim, NumberType>::get_error_update(
...
template <int dim, typename NumberType>
BlockVector<double> Solid<dim, NumberType>::get_total_solution(
...
template <int dim> struct Assembler<dim, double> : Assembler_Base<dim, double> {
  ...
  virtual void assemble_system_tangent_residual_one_cell(
      const typename DoFHandler<dim>::active_cell_iterator &cell,
      ScratchData_ASM &scratch, PerTaskData_ASM &data) override {
    // Aliases for data referenced from the Solid class
...
struct Assembler<dim, Sacado::Fad::DFad<double>>
  ...
  virtual void assemble_system_tangent_residual_one_cell(
    ...
    cell->get_dof_values(scratch.solution_total, local_dof_values.begin(),
    ...
    for (unsigned int i = 0; i < n_independent_variables; ++i)
      local_dof_values_ad[i] =
          ADNumberType(n_independent_variables, i, local_dof_values[i]);
    ...
    scratch.fe_values_ref[u_fe].get_function_gradients_from_local_dof_values(
        local_dof_values_ad, scratch.solution_grads_u_total);
    ...
    for (unsigned int q_point = 0; q_point < n_q_points; ++q_point) {
      ...
      const Tensor<2, dim, ADNumberType> F_bar =
          Physics::Elasticity::Kinematics::F_iso(F);
      const SymmetricTensor<2, dim, ADNumberType> b_bar =
          Physics::Elasticity::Kinematics::b(F_bar);
...
struct Assembler<dim, Sacado::Rad::ADvar<Sacado::Fad::DFad<double>>>
  ...
  virtual void assemble_system_tangent_residual_one_cell(
    ...
    for (unsigned int i = 0; i < n_independent_variables; ++i)
      local_dof_values_ad[i] =
          ADDerivType(n_independent_variables, i, local_dof_values[i]);
...
void Solid<dim, NumberType>::make_constraints(const int &it_nr) {
  ...
  // the -x face (ID = 1), we set up a zero-displacement condition, -y and +y
  // traction free boundary condition (don't need to take care); -z and +z faces
  // (ID = 2) are not allowed to move along z axis so that it is a plane strain
  // problem. Finally, as described earlier, +x face (ID = 11) has an the
  // applied distributed shear force (converted by total force per unit area)
  // which needs to be taken care as an inhomogeneous Newmann boundary
  // condition.
  //
...
template <int dim, typename NumberType>
std::pair<unsigned int, double> Solid<dim, NumberType>::solve_linear_system(
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (parameters.automatic_differentiation_order == 0) {
      std::cout << "Assembly method: Residual and linearisation are computed "
                   "manually."
                << std::endl;
      ...
      std::cout << "Assembly method: Residual computed manually; linearisation "
                   "performed using AD."
                << std::endl;
      ...
      std::cout
          << "Assembly method: Residual and linearisation computed using AD."
          << std::endl;
      ...
      AssertThrow(
          false,
          ExcMessage(
              "The selected assembly method is not supported. "
